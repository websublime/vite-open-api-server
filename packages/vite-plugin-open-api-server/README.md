...
The plugin integrates with Vue DevTools to provide a custom inspector for browsing endpoints and debugging.

### Installation

Install the DevTools plugin in your Vue application:
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `proxyPath` | `string` | `'/api'` | Base path for the mock server API |
| `verbose` | `boolean` | `false` | Enable verbose logging |
| `maxRetries` | `number` | `15` | Maximum retries to fetch registry |
| `retryDelay` | `number` | `1000` | Delay between retries (ms) |

### Debugging with Console

When the DevTools plugin is installed, a global object is exposed at `window.__VITE_OPENAPI_SERVER__` for console debugging:
...
window.__VITE_OPENAPI_SERVER__.getStats()
// Returns: { total: 19, withHandlers: 5, withSeeds: 3, autoGenerated: 14, ... }

// List all endpoints as a table
window.__VITE_OPENAPI_SERVER__.listEndpoints()
// Outputs a formatted console table

// View request log
window.__VITE_OPENAPI_SERVER__.getRequestLog()
// Returns: [{ id, method, path, status, duration, timestamp, ... }]

// Clear request log
window.__VITE_OPENAPI_SERVER__.clearRequestLog()

// Refresh registry from server
await window.__VITE_OPENAPI_SERVER__.refresh()
```
...
| Property | Type | Description |
|----------|------|-------------|
| `registry` | `RegistryData \| null` | Current registry data |
| `handlers` | `Map<string, boolean>` | Map of operationId to hasHandler |
| `requestLog` | `RequestLogEntry[]` | Recent API requests (max 100) |
| `version` | `string` | Plugin version |
| `proxyPath` | `string` | Configured proxy path |

### Global State Methods

| Method | Returns | Description |
|--------|---------|-------------|
| `getEndpoints()` | `EndpointData[]` | Get all endpoints as array |
| `getEndpoint(operationId)` | `EndpointData \| undefined` | Get specific endpoint |
| `getHandlerStatus(operationId)` | `boolean` | Check if endpoint has handler |
| `getSchema(name)` | `unknown \| undefined` | Get schema by name |
| `getStats()` | `RegistryStats` | Get registry statistics |
| `listEndpoints()` | `void` | Log endpoints table to console |
| `logRequest(entry)` | `void` | Add entry to request log |
| `clearRequestLog()` | `void` | Clear the request log |
| `getRequestLog()` | `RequestLogEntry[]` | Get copy of request log |
| `refresh()` | `Promise<void>` | Refresh registry from server |
